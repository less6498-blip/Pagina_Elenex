...
function renderResumen() {
    ...
    <div class="d-flex gap-3 align-items-center mb-3" data-id="${item.id}">
        <div class="flex-shrink-0" style="position:relative;width:64px;height:64px;overflow:visible;">
            <img src="${item.image || ''}" alt="${item.name}"
                 style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid #eee;display:block;"
        ...
        <div class="d-flex align-items-center flex-shrink-0"
            ...
            <button onclick="cambiarCantidad('${item.id}', -1)"
                    style="width:30px;height:30px;background:none;border:none;
                           font-size:16px;cursor:pointer;line-height:1;"
                    aria-label="Reducir cantidad">−</button>
            <span id="qty-${item.id}"
                  style="min-width:28px;text-align:center;font-size:13px;font-weight:600;">
                ${item.quantity}
            </span>
            <button onclick="cambiarCantidad('${item.id}', 1)"
                    style="width:30px;height:30px;background:none;border:none;
    ...
    try {
        const res = await fetch(URL_PROCESAR, {
            ...
            body: JSON.stringify({
                ...
                cart_items:   JSON.stringify(getCart()),
            }),
        });
        ...
        if (data.success) {
            ...
            mostrarError(data.error || data.message || 'Error al procesar');
            btn.disabled  = false;
            btn.innerHTML = '✓ Confirmar pedido — ' + document.getElementById('resumen-total').textContent;
        }
    } catch (e) {
